...
vector<pair<ll,ll>>Adj[N];vector<bool>used(N,false);
bool Visited[N]={false};
vector<ll> Dis(N,1e18);vector<pair<ll,ll>>A;
...
void solve_Dijkstra() {
   ...
   for(int i =1;i<=n;i++){
       for(auto u:Adj[i]) {
           ...
           ll d1=l-Dis[i];
           ll d2=l-Dis[x];
           if(0<d1&&d1<y&&(y-d1)+Dis[x]>l)num++;
           if(0<d2&&d2<y&&(y-d2)+Dis[i]>l)num++;
           if(0<d1&&0<d2&&(d1+d2)==y)num++;
       }
   }
    cout<<num<<endl;
}
